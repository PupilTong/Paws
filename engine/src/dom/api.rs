...
pub(crate) trait ModernDomApi<'a> {
...
pub(crate) struct ComputedStyleMap<'a> {
    node: &'a PawsElement,
    context: &'a crate::style::StyleContext,
}

impl<'a> ComputedStyleMap<'a> {
    pub(crate) fn new(node: &'a PawsElement, context: &'a crate::style::StyleContext) -> Self {
        Self { node, context }
    }
    ...
    pub(crate) fn get(&self, property_name: &str) -> Option<String> {
        self.node
