...
3. **`wasmtime-engine/src/host_impl.rs`** — add the bindgen-generated
   ...
   Preserve the ABI: non-negative `i32` return values are success
   (ids, sizes, or booleans encoded as 0/1); negative values are
   [`HostErrorCode`](engine/src/runtime.rs) variants. The two
   functions whose success value is legitimately nullable
   (`get-namespace-uri`, `get-attribute`) use `result<option<string>,
   s32>` instead so "absent" and "error" don't collide on `-1`.
